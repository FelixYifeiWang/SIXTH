...
print(f"[emotion_inference] sending to LLM...")
...
print(f"[emotion_inference] finish_reason: {choice.finish_reason}")

raw = choice.message.content or ""
raw = raw.strip()
print(f"[emotion_inference] raw response: {repr(raw)}")
...
if not raw:
    print("[emotion_inference] ERROR: empty response from model")
    exit(1)

try:
    result = json.loads(raw)
except json.JSONDecodeError as e:
    print(f"[emotion_inference] ERROR: could not parse JSON: {e}")
    print(f"[emotion_inference] content was: {repr(raw)}")
    exit(1)
...
print(f"[emotion_inference] wrote output.json:")
